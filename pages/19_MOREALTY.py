...
res = st.session_state.all_data["MOREALTY"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['MOREALTY']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for MOREALTY')
        res_place.text('')
