...
res = st.session_state.all_data["MASPTOP50"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['MASPTOP50']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for MASPTOP50')
        res_place.text('')
